...
<span class="text-[0.6rem] block pt-1 text-typography-grey {timeStyle}">{comment.createdAt.getHours() + ":" + (comment.createdAt.getMinutes() < 10 ? "0" + comment.createdAt.getMinutes() : comment.createdAt.getMinutes())}</span>